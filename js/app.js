...
if (sliderScrollItems.length > 0) {
  for (var _index3 = 0; _index3 < sliderScrollItems.length; _index3++) {
    var sliderScrollItem = sliderScrollItems[_index3];
...
var mainblock__slider = new Swiper('.slider__wrapper', {
  /*
  effect: 'fade',
  */
  autoplay: {
...
var recipes__slider = new Swiper('.slider-recipes__wrapper', {
  /*
  effect: 'fade',
  */
  autoplay: {
...
var elem = document.querySelector('.calendar__row');
var date = new Date();
createCalendar(elem, date.getFullYear(), date.getMonth() + 1);
var leftPosition = 0;
...
var minIndex = 0;
var maxIndex = 0;
var calendarPrev = document.querySelector(".calendar-block__prev");
var calendarNext = document.querySelector(".calendar-block__next");
calendarPrev.addEventListener("click", function () {
  date.setMonth(date.getMonth() - 1);

  if (minIndex == index) {
    elem.style.width = elemWidth + 250 + 'px';
    elemWidth += 250;
    translatePosition -= 250;
    elem.style.transform = "translate(".concat(translatePosition, "px,0)");
    leftPosition -= 250;
    createCalendar(elem, date.getFullYear(), date.getMonth() + 1, true);
    elem.style.left = leftPosition + 500 + 'px';
    leftPosition += 500;
  } else {
    elem.style.left = leftPosition + 250 + 'px';
    leftPosition += 250;
  }

  titleChange(date.getMonth() + 1, date.getFullYear());
  index--;
...
function titleChange(month, year) {
  var dateMonth = document.querySelector(".calendar-block__date_month");
  var dateYear = document.querySelector(".calendar-block__date_year");
  var monthName;
  ...
  dateMonth.innerHTML = monthName;
  dateYear.innerHTML = year;
}

function createCalendar(elem, year, month) {
  var prev = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  titleChange(month, year);
  var table = document.createElement('table');
  table.classList.add("calendar__table");
  table.classList.add("load");
  var days = document.createElement('tr');
  var mn = document.createElement('th');
  mn.textContent = 'Mo';
  days.append(mn);
  var tu = document.createElement('th');
  tu.textContent = 'Tu';
  days.append(tu);
  var wd = document.createElement('th');
  wd.textContent = 'We';
  days.append(wd);
  ...
  fr.textContent = 'Fr';
  days.append(fr);
  var st = document.createElement('th');
  st.textContent = 'Sa';
  days.append(st);
  ...
  var dayNum = date.getDay() == 0 ? 6 : date.getDay() - 1;
  ...
  var weekNum = Math.ceil(lastDate / 7);

  for (var _i = 0; _i < weekNum; _i++) {
  ...
  var tr = 1;
  var td = dayNum;
  var previousDay = new Date(year, month - 1, -dayNum + 1).getDate();

  for (var _i2 = 0; _i2 < dayNum; _i2++) {
    var emptyTd = document.createElement('td');
    emptyTd.classList.add("unavailable");
    emptyTd.textContent = previousDay;
    table.rows[1].append(emptyTd);
    previousDay++;
  }
  ...
  for (var _day = 1; _day <= lastDate; _day++) {
    ...
    if (table.rows[tr]) {
      table.rows[tr].append(newTd);
      table.rows[tr].cells[td].textContent = _day;
    }
  ...
  while (td <= 6) {
    ...
    _newTd.classList.add("unavailable");

    _newTd.textContent = day;

    if (table.rows[tr]) {
      table.rows[tr].append(_newTd);
    }
...
var likesButtons = document.querySelectorAll('.header-comment__likes');

var _iterator = _createForOfIteratorHelper(likesButtons),
    _step;

try {
  var _loop11 = function _loop11() {
    var likesButton = _step.value;
    likesButton.addEventListener("click", function () {
      var likesValue = likesButton.querySelector('.header-comment__likes_value');
      likesValue.innerHTML = +likesValue.innerHTML + 1;
    });
  };
...
var subCommentsBlocks = document.querySelectorAll('.comment__sub-comments');

var _iterator2 = _createForOfIteratorHelper(subCommentsBlocks),
    _step2;

try {
  var _loop12 = function _loop12() {
    var subCommentsBlock = _step2.value;
    var showButton = subCommentsBlock.querySelector('.comment__show');
    showButton.addEventListener("click", function () {
      var subComments = subCommentsBlock.querySelectorAll('.sub-comment');

      var _iterator3 = _createForOfIteratorHelper(subComments),
          _step3;

      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
          var subComment = _step3.value;
...
function body_lock_add(delay) {
  ...
  if (unlock) {
    ...
    for (var _index5 = 0; _index5 < lock_padding.length; _index5++) {
      var _el2 = lock_padding[_index5];
...
if (title) {
  for (var _index6 = 0; _index6 < title.length; _index6++) {
    var _el3 = title[_index6];
    var txt = _el3.innerHTML;
    var txt_words = txt.replace('  ', ' ').split(' ');
    var new_title = '';

    for (var _index7 = 0; _index7 < txt_words.length; _index7++) {
      var txt_word = txt_words[_index7];
      var len = txt_word.length;
      new_title = new_title + '<p>';

      for (var _index8 = 0; _index8 < len; _index8++) {
        var it = txt_word.substr(_index8, 1);
...
var _loop = function _loop(_index9) {
  var tab = tabs[_index9];
  var tabs_items = tab.querySelectorAll("._tabs-item");
  var tabs_blocks = tab.querySelectorAll("._tabs-block");

  var _loop13 = function _loop13(_index42) {
    var tabs_item = tabs_items[_index42];
    tabs_item.addEventListener("click", function (e) {
      for (var _index43 = 0; _index43 < tabs_items.length; _index43++) {
        var _tabs_item = tabs_items[_index43];

        _tabs_item.classList.remove('_active');

        tabs_blocks[_index43].classList.remove('_active');
      }

      tabs_item.classList.add('_active');

      tabs_blocks[_index42].classList.add('_active');
...
for (var _index9 = 0; _index9 < tabs.length; _index9++) {
  _loop(_index9);
} //=================
//Spollers
...
if (spollers.length > 0) {
  var _loop2 = function _loop2(_index10) {
    var spoller = spollers[_index10];
    spoller.addEventListener("click", function (e) {
      if (spollersGo) {
        ...
        if (spoller.closest('._spollers').classList.contains('_one')) {
          ...
          for (var _i3 = 0; _i3 < curent_spollers.length; _i3++) {
            var _el4 = curent_spollers[_i3];
  ...
  for (var _index10 = 0; _index10 < spollers.length; _index10++) {
    _loop2(_index10);
  }
} //=================
//Gallery
...
function gallery_init() {
  for (var _index11 = 0; _index11 < gallery.length; _index11++) {
    var _el5 = gallery[_index11];
    lightGallery(_el5, {
      counter: false,
      selector: 'a',
      download: false
...
function digi_animate(digi_animate) {
  if (digi_animate.length > 0) {
    for (var _index12 = 0; _index12 < digi_animate.length; _index12++) {
      var _el7 = digi_animate[_index12];
...
var _loop3 = function _loop3(_index13) {
  var el = popup_link[_index13];
...
for (var _index13 = 0; _index13 < popup_link.length; _index13++) {
  _loop3(_index13);
}

for (var _index14 = 0; _index14 < popups.length; _index14++) {
  var popup = popups[_index14];
...
function popup_close(item) {
  ...
  if (unlock) {
    if (!item) {
      for (var _index15 = 0; _index15 < popups.length; _index15++) {
        var _popup = popups[_index15];
...
if (popup_close_icon) {
  var _loop4 = function _loop4(_index16) {
    var el = popup_close_icon[_index16];
    el.addEventListener('click', function () {
      popup_close(el.closest('.popup'));
    });
  };

  for (var _index16 = 0; _index16 < popup_close_icon.length; _index16++) {
    _loop4(_index16);
  }
}
...
function select_actions(original, select) {
  ...
  var _loop6 = function _loop6(_index26) {
    var select_option = select_options[_index26];
    ...
    select_option.addEventListener('click', function () {
      for (var _index27 = 0; _index27 < select_options.length; _index27++) {
        var _el11 = select_options[_index27];
...
function select_get_options(select_options) {
  if (select_options) {
    ...
    for (var _index28 = 0; _index28 < select_options.length; _index28++) {
      var select_option = select_options[_index28];
...
function selects_update_all() {
  ...
  if (selects) {
    for (var _index29 = 0; _index29 < selects.length; _index29++) {
      var select = selects[_index29];
      select_item(select);
    }
  }
} //Placeholers
...
function inputs_init(inputs) {
  if (inputs.length > 0) {
    var _loop7 = function _loop7(_index30) {
      var input = inputs[_index30];
      var input_g_value = input.getAttribute('data-value');
    ...
    for (var _index30 = 0; _index30 < inputs.length; _index30++) {
      _loop7(_index30);
    }
  }
}
...
if (quantityButtons.length > 0) {
  var _loop8 = function _loop8(_index31) {
    var quantityButton = quantityButtons[_index31];
  ...
  for (var _index31 = 0; _index31 < quantityButtons.length; _index31++) {
    _loop8(_index31);
  }
} //RANGE
...
function scroll_scroll() {
  ...
  if (scr_blocks.length > 0) {
    for (var _index32 = 0; _index32 < scr_blocks.length; _index32++) {
      var block = scr_blocks[_index32];
  ...
  if (scr_items.length > 0) {
    for (var _index33 = 0; _index33 < scr_items.length; _index33++) {
      var scr_item = scr_items[_index33];
...
function scroll_lazy(scr_item) {
  ...
  if (lazy_src.length > 0) {
    for (var _index34 = 0; _index34 < lazy_src.length; _index34++) {
      var _el12 = lazy_src[_index34];
  ...
  if (lazy_srcset.length > 0) {
    for (var _index35 = 0; _index35 < lazy_srcset.length; _index35++) {
      var _el13 = lazy_srcset[_index35];
  ...
  window.addEventListener('scroll', function (el) {
    ...
    if (old_current_link) {
      for (var _index37 = 0; _index37 < old_current_link.length; _index37++) {
        var _el14 = old_current_link[_index37];
